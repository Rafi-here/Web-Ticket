...
@section('content')
                ...
                <div class="border-b border-gray-200 dark:border-gray-700 mb-6">
                    <nav class="flex space-x-8" x-data="{ tab: 'general' }">
                        ...
                        <button type="button" @click="tab = 'whatsapp'"
                            :class="{ 'border-blue-500 text-blue-600': tab === 'whatsapp' }"
                            class="py-2 px-1 border-b-2 border-transparent font-medium text-sm hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none">
                            WhatsApp
                        </button>
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="flex-1">
                                        <input type="file" name="logo_image" id="logo_image" accept="image/*"
                                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white">
                                        <p class="mt-1 text-xs text-gray-500">Upload gambar logo (format: JPG, PNG, maks: 2MB)</p>
                                    </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Jika diaktifkan, user bisa memesan tiket via WhatsApp</p>
                                </div>
                            ...
                            <div>
                                <label for="whatsapp_message_template" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                ...
                                <div class="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                                    <h4 class="text-sm font-semibold text-blue-800 dark:text-blue-300 mb-2">Variable yang tersedia:</h4>
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                                        <div><code class="bg-blue-100 dark:bg-blue-800 px-1 py-0.5 rounded">{name}</code> - Nama user</div>
                                        <div><code class="bg-blue-100 dark:bg-blue-800 px-1 py-0.5 rounded">{email}</code> - Email user</div>
                                        <div><code class="bg-blue-100 dark:bg-blue-800 px-1 py-0.5 rounded">{event_title}</code> - Judul event</div>
                                        <div><code class="bg-blue-100 dark:bg-blue-800 px-1 py-0.5 rounded">{event_date}</code> - Tanggal event</div>
                                        <div><code class="bg-blue-100 dark:bg-blue-800 px-1 py-0.5 rounded">{event_time}</code> - Waktu event</div>
                                        <div><code class="bg-blue-100 dark:bg-blue-800 px-1 py-0.5 rounded">{ticket_details}</code> - Detail tiket</div>
                                        <div><code class="bg-blue-100 dark:bg-blue-800 px-1 py-0.5 rounded">{total_price}</code> - Total harga</div>
                                        <div><code class="bg-blue-100 dark:bg-blue-800 px-1 py-0.5 rounded">{quantity}</code> - Jumlah tiket</div>
                                    </div>
                                </div>

                                <!-- Preview Button -->
                                <div class="mt-4">
                                    <button type="button" id="preview-wa-btn" class="text-sm text-blue-600 hover:text-blue-700">
                                        👁️ Lihat Preview Pesan
                                    </button>
                                    <div id="preview-wa-box" class="mt-2 p-4 bg-gray-100 dark:bg-gray-700 rounded-lg hidden">
                                        <p class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap" id="preview-wa-text"></p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    @push('scripts')
        <script>
            ...
            // WhatsApp Preview
            const previewBtn = document.getElementById('preview-wa-btn');
            if (previewBtn) {
                previewBtn.addEventListener('click', function() {
                    const template = document.getElementById('whatsapp_message_template').value;
                    const previewBox = document.getElementById('preview-wa-box');
                    const previewText = document.getElementById('preview-wa-text');
                    ...
                    let preview = template
                        ...
                        .replace(/{event_title}/g, sample.event_title)
                        .replace(/{event_date}/g, sample.event_date)
                        .replace(/{event_time}/g, sample.event_time)
                        .replace(/{ticket_details}/g, sample.ticket_details)
                        .replace(/{total_price}/g, sample.total_price)
                        .replace(/{quantity}/g, sample.quantity);
