...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-12">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Success Message -->
        @if(session('success'))
        <div class="mb-6 p-4 bg-green-100 dark:bg-green-900/30 border border-green-400 dark:border-green-700 rounded-lg text-green-700 dark:text-green-400">
            {{ session('success') }}
        </div>
        @endif

        <!-- E-Ticket Card -->
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl overflow-hidden border-2 border-blue-500 dark:border-blue-400">
            <!-- Ticket Header -->
                    ...
                    <div class="md:col-span-2">
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                ...
                                <p class="font-bold text-gray-900 dark:text-white">{{ $ticket->showtime->film->title }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Bioskop</p>
                                <p class="font-bold text-gray-900 dark:text-white">{{ $ticket->showtime->cinema->name }}</p>
                            ...
                            <div>
                                ...
                                @if($ticket->status == 'paid')
                                    <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs font-semibold">LUNAS</span>
                                @elseif($ticket->status == 'pending')
                                    <span class="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs font-semibold">PENDING</span>
                                @else
                                    <span class="px-2 py-1 bg-red-100 text-red-800 rounded-full text-xs font-semibold">EXPIRED</span>
                                @endif
                            </div>
                        </div>
                            ...
                            <a href="{{ route('ticket.download', $ticket->booking_code) }}" 
                               class="flex-1 px-6 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white font-semibold rounded-lg hover:from-green-700 hover:to-green-800 transition-all duration-300 transform hover:scale-105 shadow-lg flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M12 4v12m0 0l-3-3m3 3l3-3" />
                                </svg>
                                Download Tiket (PNG)
                            </a>
                            <a href="{{ route('home') }}" 
            ...
            <!-- Ticket Footer -->
            <div class="p-4 bg-gray-100 dark:bg-gray-900 text-center text-sm text-gray-600 dark:text-gray-400 border-t border-gray-200 dark:border-gray-700">
                <i class="fas fa-ticket-alt mr-2"></i>
                Terima kasih telah memesan tiket di TIX. Selamat menonton!
            </div>
        </div>
    </div>
</div>
